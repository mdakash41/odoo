...
class HospitalSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].set_param('test_app.note', self.note)
        print("test", self.product_ids.ids)
        self.env['ir.config_parameter'].set_param('test_app.product_ids', self.product_ids.ids)
        return res
    ...
    def get_values(self):
        ...
        notes = ICPSudo.get_param('test_app.note')
        product_ids = self.env['ir.config_parameter'].sudo().get_param('test_app.product_ids')
        if product_ids:
